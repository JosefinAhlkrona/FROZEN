...
!-----
    ...
    FUNCTION EffectiveViscosityNodal( Viscosity,Density,Ux,Uy,Uz,Element, &
      ...
      INTEGER(KIND=AddrInt) :: Fnc,istat
...
!------------------------------------------------------------------------------
      IF ( isotropic ) THEN
        ...
        IF( ViscNonnewtonian ) THEN
        ! muserial=EffectiveViscosityNodal( mu, rho, Ux, Uy, Uz, &
        !      Element, Nodes, n, n, u, v, w,  muder0 )
          mu = EffectiveViscosity( mu, rho, Ux, Uy, Uz, &
              Element, Nodes, n, n, u, v, w,  muder0 )
        WRITE(111,*) Element % NodeIndexes(i),mu
