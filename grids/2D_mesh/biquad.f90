...
integer                              :: NRa      !
integer                              :: MNODE    ! nombre de points du sous-tableau (points of the subarray)

integer                              :: k,km        ! index des points du cercle (index points of the circle) 
integer                              :: l        ! compteur du nb de points reelement selectionnes (counter nb points REALLY SELECTED)
integer                              :: imin, imax, jmin, jmax ! Si on est trop près du bord (< distance R) (If you are too close to the edge (<distance R))
integer                              ::  i , j, ii   ! compteurs pour parcourir tt le domaine (counters to go tt the field)
integer                              ::  iz, jz  ! compteurs pour parcourir la sous-grille (counters to traverse the sub-grid)

real                                 :: x,y      ! coordonnees dans la sous-grille
real                                 :: dx_ls = 1! pas de la grille dans le moindre carre (grid spacing in the lower edge)
...
real,dimension(:,:),    allocatable  :: Blin     ! en input c'est la surface sur les points du cercle (input is in the surface points on the circle)
real,dimension(:,:),    allocatable  :: Bp       ! partie non vide de la matrice Blin  (pour envoie a sgelsy) (non-empty part of the matrix Blin (to send a sgelsy))
...
if (NRa <= 0) THEN
	write(6,*) "radius is not large enough to capture any points in in the grid"
	write(6,*) "R: ", R, " needs to be larger than dx: ", dx
	stop
endif
...
k=0
do j=-NRa,NRa                                ! balaye tous les points du sous tableau (sweeps all the points in the table)
   do i=-NRa,NRa
      x=real(i)*dx_ls
      y=real(j)*dx_ls
      dist=(x*x+y*y)**0.5
...
! ndata are the number of nodes in the initial mesh

do ii=1, ndata

	! this should be calculating where in the DEM the grid node should be located
...
!	stop
	...
	!	stop
		...
		write(565,*) jz, iz, k, Blin(k,1), l, Bp(l,1), dem(iz+i,jz+j)
...
! (appeal to the resolution for 1 point: lapack subroutine (see doc at end of program))
...
!mm peut changer, aini que Amm,nn & Bmm,1
	...
	write(555,*) ii, Blin(6,1)
!	stop

	coefs_blin(ii,:) = Blin(:,1)
!	write(6,*) ii, coefs_blin(ii,:)
