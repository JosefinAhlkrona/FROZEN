RECURSIVE SUBROUTINE pdd( Model,Solver,Timestep,TransientSimulation )
  ...
  LOGICAL :: Anomalies, anomaliesParam, lapsrate, TarasovPDD = .FALSE., HuybrechtsPDD = .FALSE.
  CHARACTER(LEN=MAX_NAME_LEN) :: SolverName, ch_dummy, INPATH, GRIP_TEMP_FILE, GLAC_IND_FILE, &
                              TEMP_MA_ANOM_FILE, TEMP_MJ_ANOM_FILE, PrecipMode, TempParam
  ...
  SAVE AllocationsDone, SolverName, old_body, TSURFACE, ABLSURFACE, SOLID_PRECIP, &
      ...
      ET, melt, melt_star, watchdog, PrecipMode, TempParam, Anomalies, anomaliesParam, anomaliesScaling, & 
      ...
      anamaliesJulyTempAtT94, lapsrate, sTime
  ...
  IF ( .NOT. AllocationsDone .OR. Solver % Mesh % Changed ) THEN
        ...
        IF(Anomalies) THEN
            ...
            ar4_precipSol => VariableGet( Solver % Mesh % Variables, 'preciptation' )!JOSEFIN
            ...
            ar4_annualtempSol => VariableGet( Solver % Mesh % Variables, 'annualtemp' ) !JOSEFIN
            ...
            ar4_julytempSol => VariableGet( Solver % Mesh % Variables, 'julytemp' ) !JOSEFIN
  ...
  SELECT CASE(PrecipMode)
   CASE('constant')
    ...
    precip_ma_presSol => VariableGet( Solver % Mesh % Variables, 'presentprecip' ) !JOSEFIN
    ...
    IF (Anomalies) THEN
        ar4_precipSol => VariableGet( Solver % Mesh % Variables, 'preciptation' )
            IF ( ASSOCIATED( ar4_precipSol ) ) THEN
                ar4_precip_Perm => ar4_precipSol % Perm
                ar4_precip      => ar4_precipSol % Values
                ar4_precip_DOFS = ar4_precipSol % DOFs
  ...
  SELECT CASE(TempParam)
   ...
   CASE('parametrization')
    ...
    IF (Anomalies) THEN 
        ...
        ar4_annualtempSol => VariableGet( Solver % Mesh % Variables, 'annualtemp' )
        IF ( ASSOCIATED( ar4_annualtempSol ) ) THEN
            ar4_annualtemp_Perm   => ar4_annualtempSol % Perm
            ar4_annualtemp        => ar4_annualtempSol % Values
            ar4_annualtemp_DOFs   =  ar4_annualtempSol % DOFs
        ...
        ar4_julytempSol => VariableGet( Solver % Mesh % Variables, 'julytemp' )
        IF ( ASSOCIATED( ar4_julytempSol ) ) THEN
            ar4_julytemp_Perm   => ar4_julytempSol % Perm
            ar4_julytemp        => ar4_julytempSol % Values
            ar4_julytemp_DOFs   =  ar4_julytempSol % DOFs
     ...
     ELSE IF (ACCSURFACE == 6) THEN 
       ...
       IF (Anomalies) THEN
            ...
            precip(NodeIndexes(k),0) = ( ( precip_ma_present(precip_ma_presDOFs*(precip_ma_presPerm(NodeIndexes(k))-1)+1) + &
                anomalies_precip(NodeIndexes(k)) )/YEAR_SEC ) &
                * (RHO_W/RHO)  ! mean annual precip
       ELSE
