...
!-----------------------------------------------------------
  ...
  TYPE(Variable_t), POINTER :: VarTemp, VarResidual
  INTEGER :: i, N, DIM, NBoundary, NParent, BoundaryElementNode,&
       ParentElementNode, body_id, other_body_id, material_id, istat
  REAL(KIND=dp), ALLOCATABLE :: LatentHeat(:), Density(:),ExternalHF(:),PressureMeltingPoint(:)
  REAL (KIND=dp) :: HomTemp, BasalMeltingOffset
  CHARACTER(LEN=MAX_NAME_LEN) :: TempName, SolverName
  LOGICAL ::  FirstTime = .TRUE., GotIt, stat

  SAVE FirstTime, LatentHeat, Density, DIM, N, ExternalHF, PressureMeltingPoint
  ...
  ! Allocations
  !--------------------------------
  !PRINT *, FirstTime, Model % Mesh % Changed
  ...
  IF (VarResidual % Values(VarResidual % Perm(Node)) < 0.0D00) THEN
     basalMeltingvelocity = MAX((ExternalHF(BoundaryElementNode)+HeatFlux) &
          /(LatentHeat(BoundaryElementNode) * Density(BoundaryElementNode)),BasalMeltingOffset)
 !    PRINT *, Node, basalMeltingvelocity, ExternalHF(BoundaryElementNode), HeatFlux
  ELSE
     basalMeltingvelocity = 0.0
  END IF
...
!-----------------------------------------------------------
  ...
  INTEGER :: NMax, OldNMax, NBoundary, NParent, BoundaryElementNode, ParentElementNode, &
       i, DIM, other_body_id, body_id, material_id, istat
  REAL(KIND=dp) :: TempHom, ThermalCoefficient, SlipCoefficientMax
  REAL(KIND=dp), ALLOCATABLE :: RelativeTemperature(:), TemperateSlipCoefficient(:)
  CHARACTER(LEN=MAX_NAME_LEN) :: TempName
  LOGICAL ::  GotIt, stat, Jump=.FALSE., AllocationsDone

  SAVE AllocationsDone, TemperateSlipCoefficient, RelativeTemperature, OldNMax

  !---------------
  ! Initialization
  !--------------- 
  NMax = Model % MaxElementNodes 
  IF ( (.NOT.AllocationsDone) .OR. (NMax > OldNMax)) THEN
     IF (AllocationsDone) &
          DEALLOCATE(TemperateSlipCoefficient, RelativeTemperature)
     ALLOCATE(TemperateSlipCoefficient(NMax),&  
          RelativeTemperature(NMax),&
          STAT = istat)
     IF (istat /= 0) THEN
  ...
  !-------------------------
  TempName =  GetString(ParentMaterial ,'Temperature Name', GotIt)
  CALL GetScalarLocalSolution(RelativeTemperature, TRIM(TempName) // ' Homologous', ParentElement)
  TempHom = MIN(RelativeTemperature(ParentElementNode),0.0d00)
  !---------------------------------------------------------------------
...
FUNCTION Sliding_Weertman (Model, nodenumber, y) RESULT(Bdrag)
   ...
   IMPLICIT NONE
   TYPE(Model_t) :: Model
   TYPE(Element_t), POINTER ::  BCElement, CurElement
   TYPE(ValueList_t), POINTER :: BC
   TYPE(variable_t), POINTER :: TimeVar
   INTEGER :: nodenumber, Ng
   INTEGER :: i, j, p, n, Nmax, Nn 
   REAL(KIND=dp) :: x, y, Xg, fBed, t, told, eps
   LOGICAL ::  GotIt, ComputeS
   ...
   CHARACTER(LEN=max_name_len) :: Sl_law
   REAL (KIND=dp) :: C, m, Bdrag
   REAL (KIND=dp) :: Sxx, Syy, Sxy, Snt, nx, ny, ut, ux, uy


   !Read the coefficients C and m in the sif file 
   BC => GetBC(Model % CurrentElement)  
   C = GetConstReal( BC, 'Sliding Coefficient', GotIt )
   IF (.NOT.GotIt) THEN
      CALL FATAL('USF_sliding', 'Need a Sliding Coefficient for the choosen sliding law (weertman)')
   END IF
   m = GetConstReal( BC, 'Sliding Exponent', GotIt )
   IF (.NOT.GotIt) THEN
      CALL FATAL('USF_sliding', 'Need a Sliding Exponent for the choosen sliding law (weertman)')
   END IF
   
   ! Get the variables to compute tau_b
   StressVariable => VariableGet( Model % Variables, 'Sij' )
   ...
   nx = -NormalValues(2*(NormalPerm(Nodenumber)-1) + 1)      
   ny = -NormalValues(2*(NormalPerm(Nodenumber)-1) + 2)

   ux = FlowValues( 3 *(FlowPerm(Nodenumber)-1) + 1 )
   uy = FlowValues( 3 *(FlowPerm(Nodenumber)-1) + 2 )

   ut = ABS ( ux*ny - uy*nx )

   IF ( ASSOCIATED( StressVariable ) ) THEN
       Sxx = StressValues( 4 *(StressPerm(Nodenumber)-1) + 1 )
       Syy = StressValues( 4 *(StressPerm(Nodenumber)-1) + 2 )
       Sxy = StressValues( 4 *(StressPerm(Nodenumber)-1) + 4 )

       Snt = ABS( Sxy*(nx*nx-ny*ny) - (Sxx-Syy)*nx*ny )
   END IF
   
   ! We can distinguish 4 cases:
   ! 1/ ut=0 , tau_b=0     => C=1.0e20
...
!------------ remember this -------------------------------
  ...
  visFact =2.0*(2.0D00*enhancementFactor(nodeInElement)*rateFactor)**(-1.0D00*viscosityExponent(nodeInElement))
