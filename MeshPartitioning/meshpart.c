...
#include <metis.h>
#include <stdio.h> /*added by josefin*/
                         ...
                         int *nparts, int *edgecut, idxtype *epart, idxtype *npart)
{
  ...
  idxtype *vwgt; /*Added by Josefin*/
  int nodenumber; /*Added by Josefin*/
  int options[10], pnumflag=0, wgtflag=2; /*weightflag changed from 0 to 2 by Josefin*/
  int nnbrs, nbrind[200], nbrwgt[200], maxpwgt;
  int esize, esizes[] = {-1, 3, 4, 8, 4};
  FILE *fid = 0; /* added by Josefin*/
  double weight; /* added by Josefin*/
  ...
  vwgt = idxmalloc(*nn, "METIS_MESHPARTNODAL: vwgt"); /*Added by Josefin*/
  
  printf("reading weights from file mesh.weights \n"); 
   fid = fopen("mesh.weights","r");
   printf("file opened\n");


  METIS_MeshToNodal(ne, nn, elmnts, etype, &pnumflag, xadj, adjncy);

  while(fscanf(fid, "%d %lf", &nodenumber, &weight)==2)
    {
	    weight=weight*10.0;
	    vwgt[nodenumber]=(int) weight;//*weight;
    }
    fclose(fid);
  ...
  printf("*** inside meshpart.c, introducing nodal weights ***\n");
  options[0] = 0; 
  //*options[4] = 1; /*added by josefin*/
  //*options[9] = 100;*/

  adjncy = realloc(adjncy, xadj[*nn]*sizeof(idxtype));

  options[0] = 0;
  METIS_PartGraphKway(nn, xadj, adjncy, vwgt, NULL, &wgtflag, &pnumflag, nparts, options, edgecut, npart);
  ...
  GKfree(&xadj, &adjncy, &pwgts, &vwgt, LTERM);
                        ...
                        int *nparts, int *edgecut, idxtype *epart, idxtype *npart)
{
  ...
  //*options[4] = 1; /*added by josefin*/
  //*options[9] = 100;*/
  METIS_PartGraphKway(ne, xadj, adjncy, vwgt, NULL, &wgtflag, &pnumflag, nparts, options, edgecut, epart); /*4th argument changed from NULL to vwgt by Josefin*/
  ...
  GKfree(&xadj, &adjncy, &pwgts, &nptr, &nind, &vwgt, LTERM);
